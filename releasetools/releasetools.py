...
def AddBootloaderFlash(info, input_zip):
  ...
  info.script.AppendExtra('        (')
  info.script.AppendExtra('          ui_print("TWRP detected");')
  info.script.AppendExtra('          run_program("/system/bin/mkdir", "-p", "/mnt/vendor");')
  info.script.AppendExtra('          run_program("/system/bin/mount", "-o", "rw,remount", "/external_sd");')
  info.script.AppendExtra('          run_program("/system/bin/ln", "-s", "/external_sd", "' + NX_FILES + '");')
  info.script.AppendExtra('        ),')
  info.script.AppendExtra('        (')
  info.script.AppendExtra('          ui_print("Assuming stock recovery--report to maintainer if not the case");')
  info.script.AppendExtra('          run_program("/system/bin/mkdir", "-p", "' + NX_FILES + '");')
  info.script.AppendExtra('          run_program("/system/bin/mount", "-t", "vfat", "/dev/block/by-name/hos_data", "' + NX_FILES + '");')
  info.script.AppendExtra('        )')
  info.script.AppendExtra('      );')
  ...
  info.script.AppendExtra('          package_extract_file("firmware-update/android.ini", "/external_sd/bootloader/ini/android.ini");')
  info.script.AppendExtra('          run_program("/system/bin/mkdir", "-p", "/tmp/install/bin");')
